  ...
  return (
    <div className="min-h-screen bg-[#0c0e12] flex flex-col justify-center items-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#11141c] border border-[#1f2533] rounded-2xl p-8 sm:p-10 shadow-2xl relative z-10">
        ...
        {/* Credentials Note Banner for evaluators */}
        <div className="mb-6 p-3.5 rounded-xl bg-[#141822] border border-[#232a3b] text-xs text-[#94a3b8] flex items-start gap-2.5">
          <CheckCircle2 className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" style={{ color: accentColor }} />
          <div>
            <span className="font-semibold text-white block mb-0.5">Quick Evaluation Credentials</span>
            <span>Pre-filled with default secure admin credentials. Click "Authenticate & Enter" to access dashboard.</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-3 rounded-lg text-sm font-semibold text-[#0c0e12] transition-all transform active:scale-95 disabled:opacity-50 cursor-pointer shadow-md"
            style={{ backgroundColor: accentColor }}
          >
